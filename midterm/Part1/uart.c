...
int uart_init()
{
  ...
  //uart[1].base = (char *)(1000A000);
  //uart[2].base = (char *)(1000B000);
  //uart[3].base = (char *)(1000C000);
}

int ugetc(UART *up)
{
  ksleep(*(up->base + FR) & RXFE);
  return *(up->base + DR);
}

int uputc(UART *up, char c)
{
  ksleep(*(up->base+FR) & TXFF);
...
/*int uartDriver()
{
  ...
  uprints(up, "enter to the uart driver");
  ugets(up, line);
  uprints(up, line);
}*/
